...
public class ShopSO : MonoBehaviour
{
    ...
    [SerializeField]
    private List<InventoryItem> _buyableItemsContainers = new List<InventoryItem>();
    ...
    private void RemoveFromBuyable(ItemContainer itemContainer, InventoryItem inventoryItem)
    {
        itemContainer.RemoveItem(inventoryItem);
        if (itemContainer.Items.Count <= 0)
        {
            _buyableItemsContainers.Remove(inventoryItem);
        }
    }
    ...
    {
        if (itemSlot.InventoryItem == null && itemSlot.ItemContainer == null)
        {
            return;
        }
    ...
    public void BuySelectedItemFromItemContainer()
    {
        ...
        InventoryItem itemToBuy = itemContainer.GetFirstSOItem();
        if (itemToBuy == null)
        {
            return;
        }
        if (_activePlayerInventory.Money < itemToBuy.GetPrice())
        {
            Debug.Log("You dont have enough money");
            return;
        }
        Debug.Log("You buy item for: " + itemToBuy.GetPrice());
        RemoveFromBuyable(itemContainer, itemToBuy);

        _activePlayerInventory.AddItem(itemToBuy);
        SetupPlayerSlots();
        SetupShopSlots();
        _activePlayerInventory.ChangeMoneyValue(-(itemToBuy.GetPrice()));
    }
    ...
    public void SellSelectedItemFromItemContainer()
    {
        ...
        ItemContainer itemContainer = _selectedItemSlot.ItemContainer;
        InventoryItem itemToSell = itemContainer.GetFirstSOItem();
        if (_activePlayerInventory.ItemContainers == null || itemToSell == null)
        {
            return;
        }
        Debug.Log("You sell item for: " + itemToSell.GetPrice());
        _activePlayerInventory.ChangeMoneyValue(itemToSell.GetPrice());

        _activePlayerInventory.RemoveItem(itemContainer);
